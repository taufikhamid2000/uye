<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Create ' . ucfirst($type) . ' Listing') }}
        </h2>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto py-12">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg">
            <div class="p-6">
                <form action="{{ route('listings.store', ['type' => $type]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <!-- Title -->
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="availability" name="availability" class="block mt-1 w-full border-gray-300 dark:bg-gray-700 dark:border-gray-600">
                            <option value="1" {{ old('availability') == '1' ? 'selected' : '' }}>{{ __('Available') }}</option>
                            <option value="0" {{ old('availability') == '0' ? 'selected' : '' }}>{{ __('Not Available') }}</option>
                        </select>
